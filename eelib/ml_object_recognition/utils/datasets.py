...
from eelib.ml_object_recognition.utils.utils import xyxy2xywh, xywh2xyxy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, dataset_id, label_map, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.labels = []
        bounding_box_map = {
            frame_id: list(bounding_boxes) for frame_id, bounding_boxes in
            groupby(store.get_bounding_boxes_for_object_recognition_dataset_by_id(dataset_id),  key=lambda x: x.frame_id)
        }
        # print(bounding_box_map)
        for frame in store.get_frames_for_object_recognition_dataset_by_id(dataset_id):
            self.img_files.append(frame.path)
            
            label = []
            if frame.id in bounding_box_map:
                bounding_boxes = bounding_box_map[frame.id]
                for box in bounding_boxes:
                    label.append(np.array([label_map[box.label], box.x, box.y, box.w, box.h]))
        ...
        assert n > 0, 'No images found in dataset'
        ...
        #     pbar = tqdm(self.labels, desc='Caching labels')
        #     nm, nf, ne, ns, nd = 0, 0, 0, 0, 0  # number missing, found, empty, datasubset, duplicate
        #     for i, label in enumerate(pbar):
        #         try:
        #             l = np.array([x.split() for x in label.splitlines()], dtype=np.float32)
        #         except:
