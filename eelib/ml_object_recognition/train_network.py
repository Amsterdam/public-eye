...
import eelib.ml_object_recognition.test as test # import test.py to get mAP after each epoch
from eelib.ml_object_recognition.models import *
from eelib.ml_object_recognition.utils.datasets import *
from eelib.ml_object_recognition.utils.utils import *
from eelib.store_utils import store_run
...
wdir = os.path.join(os.environ['EAGLE_EYE_PATH'], 'files', 'models')  # weights dir
...
hyp_default = {'giou': 3.54,  # giou loss gain
...
def train(
    ...
    # allow job arguments to overwrite hyper arguments
    hyp = {**hyp_default, **opt}
    ...
    weights = None # opt.weights  # initial training weights
    ...
    # Initialize model
    if 'pretrained_model_id' in opt and opt["pretrained_model_id"]:
        print("Loading pretrained model...")
        pretrained_model = store.get_model_by_id(opt["pretrained_model_id"])
        load_darknet_weights(model, pretrained_model.path)

    model_filename = os.path.join(wdir, 'darknet_' + str(uuid.uuid4()) + '.weights')
    ...
    for idx, label_id in enumerate(store.get_unique_labels_from_bounding_boxes_for_dataset(opt['train_dataset_id'])):
        store.insert_selected_label_if_not_exists(model_id, label_id, idx)

    labels = store.get_selected_labels_by_model_id(model_id)

    label_map = {
        label.name: store.get_selected_label(model_id, label.id).index
        for label in labels
    }

    # Configure run
    nc = int(len(labels))  # number of classes
    ...
    start_epoch = 0
    best_fitness = 0.0
    best_scores = [{'name': 'Precision', 'value': 0.0}, {'name': 'Recall', 'value': 0.0}, {'name': 'mAP', 'value': 0.0}, {'name': 'F1', 'value': 0.0}]
    ...
    # Dataset
    # this has to be store in the file

    # map should maby be hardcoded in the data
    

    dataset = LoadImagesAndLabels(opt["train_dataset_id"],
                                  label_map,
                                  img_size,
                                  batch_size,
                                  augment=True,
    ...
    # Rect requires a shapes file, perhaps needs to be fixed
    testloader = torch.utils.data.DataLoader(LoadImagesAndLabels(opt["val_dataset_id"],
                                                                 label_map, img_size_test, 1,
                                                                 hyp=hyp,
                                                                 rect=False,
                                                                 cache_images=opt['cache_images'],
                                                                 single_cls=nc == 1),
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        with open(log_file_path, 'a') as log_file:
            log_file.write("{},{},{},{},{},{},{},{}\n".format(results[0], results[1],
                results[2], results[3], mloss[0], mloss[1], mloss[2], mloss[3]))

        if fi > best_fitness:
            best_fitness = fi
            best_scores = [
                {'name': 'Precision', 'value': results[0]},
                {'name': 'Recall', 'value': results[1]},
                {'name': 'mAP', 'value': results[2]},
                {'name': 'F1', 'value': results[3]}
            ]
        ...
        if save:
            ...
            if best_fitness == fi:
                ...
                save_current_model_weights(model, model_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='yolov3-garbage/cfg/garb.data', help='*.data path')
