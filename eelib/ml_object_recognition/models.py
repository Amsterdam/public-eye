...
from eelib.ml_object_recognition.utils.google_utils import *
from eelib.ml_object_recognition.utils.parse_config import *
...
def convert(cfg='cfg/yolov3-spp.cfg', weights='weights/yolov3-spp.weights'):
    ...
    if weights.endswith('.pt'):  # if PyTorch format
        ...
        save_weights(model, path=path, cutoff=-1)
...
def save_as_weights(model, pt_path, output_path):
    model.load_state_dict(torch.load(pt_path, map_location='cpu')['model'])
    save_weights(model, path=output_path, cutoff=-1)
    print('saved weights')

def save_current_model_weights(model, output_path):
    save_weights(model, path=output_path, cutoff=-1)
    print('saved weights')
